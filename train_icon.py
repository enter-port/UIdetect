...
def train_one_epoch(model, train_loader, epoch, optimizer, scheduler, device, writer, cat, input_shape):
    ...
    for images, hms_true, whs_true, offsets_true, offset_masks_true, pre_hm, pre_wh, pre_offset, _ in tbar:
...
def main():
    ...
    now =  datetime.now()
    weight_path = "./logs/icon/{}-{}-{}-{}-{}-{}/weights".format(
        now.year, now.month, now.day, now.hour, now.minute, now.second)
    summary_path = "./logs/icon/{}-{}-{}-{}-{}-{}/summary".format(
        now.year, now.month, now.day, now.hour, now.minute, now.second)
