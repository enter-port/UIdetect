...
class CenterBoxNet(nn.Module):
    def __init__(self, backbone, num_classes=20):
        ...
        Args:
            ...
            num_classes: int
        """
        super(CenterBoxNet, self).__init__()
        ...
        # transform preprocessed bbox into h/4, w/4, 64
        self.box = CenterNetBBox()
        
        # merge two h/4, w/4, 64 into h/4, w/4, 64
        self.merge = CenterNetMerge()
...
class CenterNetBBox(nn.Module):
    def __init__(self, in_channels=8, out_channels=64, bn_momentum=0.1):
        super(CenterNetBBox, self).__init__()
        ...
        self.wh_conv = nn.Sequential(
            nn.Conv2d(2, in_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(in_channels, momentum=bn_momentum),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, hm, wh, offset):
        hm_out = self.hm_conv(hm)
        wh_out = self.wh_conv(wh)
        offset_out = self.offset_conv(offset)

        concatenated = torch.cat([hm_out, wh_out, offset_out], dim=1)
        output = self.final_conv(concatenated)

        return output
    
class CenterNetMerge(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, bn_momentum=0.1):
        super(CenterNetMerge, self).__init__()
